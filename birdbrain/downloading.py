...
from birdbrain.paths import DATA_DIR, PROJECT_DIR, ASSETS_DIR, ensure_dir
...
def get_canary_data():
    ...
    if len(glob(DATA_DIR / "processed/canary/delineations/*.img")) > 1:
        print("Data already download")
        return
    print("Downloading data")

    # which images to get and where to save them
    dl_output = DATA_DIR / "raw/canary/"
    img_output = DATA_DIR / "processed/canary/"
    images = [
...
def get_starling_data(dset_dir):
    if len(list(dset_dir.glob("delineations/*.img"))) > 1:
        print("Data already download")
        return

    print("Downloading data")
    dl_output = DATA_DIR / "raw/starling/"  # data download
    img_output = DATA_DIR / "processed/starling/"  # processed save spot
    ...
    img_files = {
        list(dl_output.glob("ATLAS_starling/*.img"))
        + list(dl_output.glob("ATLAS_starling/*.hdr"))
    }
    for img_file in img_files:
        shutil.copy(img_file, img_output / img_file.name)
...
def get_zebra_finch_data(password):
    if len(glob(DATA_DIR / "processed/zebra_finch/delineations/*.img")) > 1:
        print("Data already download")
        return
    if password is None:
        raise ValueError(
    ...
    print("Downloading data")
    dl_output = DATA_DIR / "raw/zebra_finch/"  # data download
    img_output = DATA_DIR / "processed/zebra_finch/"  # processed save spot
    ...
    atlas_img = DATA_DIR / "raw/zebra_finch/atlas/atlas.img"
    atlas_hdr = DATA_DIR / "raw/zebra_finch/atlas/atlas.hdr"
    brain_del_img = DATA_DIR / "raw/zebra_finch/atlas/brain_delineations.img"
    brain_del_hdr = DATA_DIR / "raw/zebra_finch/atlas/brain_delineations.hdr"
    nuc_del_img = DATA_DIR / "raw/zebra_finch/atlas/nuclei_delineations.img"
    nuc_del_hdr = DATA_DIR / "raw/zebra_finch/atlas/nuclei_delineations.hdr"
...
def get_pigeon_data():
    ...
    if len(glob(DATA_DIR / "processed/pigeon/delineations/*.img")) > 1:
        print("Data already download")
        return SYSTEMS_DELINEATIONS
    print("Downloading data")
    dl_output = DATA_DIR / "raw/pigeon/"  # data download
    img_output = DATA_DIR / "processed/pigeon/"  # processed save spot
...
def get_mustached_bat_data():
    ...
    if len(glob(DATA_DIR / "processed/mustached_bat/delineations/*.nii")) > 1:
        print("Data already download")
    ...
    dl_output = DATA_DIR / "raw/mustached_bat/"
    img_output = DATA_DIR / "processed/mustached_bat/"
    # make sure directories all exist
    ensure_dir(dl_output)
    ensure_dir(img_output)
    ensure_dir(img_output / "delineations/")
